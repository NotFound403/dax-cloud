...
public class RSA {
    ...
    public static void genKeyPair(String filePath) {
        ...
        try {
            ...
            try {
                // 得到公钥字符串
                byte[] publicKeyByte = Base64.encode(publicKey.getEncoded());
                // 得到私钥字符串
                byte[] privateKeyByte = Base64.encode(privateKey.getEncoded());
                // 将密钥对写入到文件
                FileWriter pubfw = new FileWriter(filePath + "/publicKey.keystore");
                FileWriter prifw = new FileWriter(filePath + "/privateKey.keystore");
                BufferedWriter pubbw = new BufferedWriter(pubfw);
                BufferedWriter pribw = new BufferedWriter(prifw);
                pubbw.write(new String(publicKeyByte));
                pribw.write(new String(privateKeyByte));
                pubbw.flush();
                pubbw.close();
                pubfw.close();
                pribw.flush();
